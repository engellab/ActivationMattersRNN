    ...
    for activation_name in activations_list:
        ...
        for constrained in constrained_list:
            ...
            for shuffle in shuffle_list:
                ...
                RNN_trajectories = get_traj(dataset=DF_dict[dkeys[0]][dkeys[1]],
                                       task=task,
                ...
                # get global PCs
                X = np.vstack([trajectory.reshape(-1, trajectory.shape[1] * trajectory.shape[-1]) for trajectory in RNN_trajectories[:n_nets]])
