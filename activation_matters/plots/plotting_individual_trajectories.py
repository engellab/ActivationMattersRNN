...
from activation_matters.plots.ploting_utils import interpolate_color, plot_trajectories
import hydra
from activation_matters.plots.style.style_setup import set_up_plotting_styles
import pickle

@hydra.main(version_base="1.3", config_path=f"../../configs", config_name=f'base')
def plot_individual_trajectories(cfg):
    show = False
    save = True
    n_nets_to_plot = 3
    ...
    for k, legend in enumerate(legends):
        if not "shuffled=True" in legend:
            RNN_trajectories_processed = data_dict["RNN_trajectories_processed"]
            for i in range(n_nets_to_plot):
                for axes in [[0, 1, 2], [0, 2, 3], [1, 2, 3]]:
                    path = os.path.join(img_save_folder, f"{taskname}_trajectories_{legend}_{axes}_{i}.pdf")
                    trajectories_projected = RNN_trajectories_processed[inds_list[k][i]]

                    plot_trajectories(trajectories=trajectories_projected,
                                      axes=axes[:2],
                                      legend=legend,
                                      save=save, path=path,
